...
const ProductFilters = ({
  categories = ["Beverages", "Dairy", "Snacks", "Fruits", "Vegetables", "Grains"],
  onFilterChange = () => {},
  onSortChange = () => {},
  activeFilters = {},
}) => {
  ...
  const getActiveFilterCount = () => {
    let count = 0
    if (safeActiveFilters.categories?.length) count += safeActiveFilters.categories.length
    if (safeActiveFilters.nutrition) count += Object.keys(safeActiveFilters.nutrition).length
    return count
  }

  return (
    <div className="bg-background border rounded-lg shadow-sm w-full max-w-screen-lg mx-auto mb-6 relative">
      {/* Filter Bar */}
      <div className="flex items-center justify-between p-3 gap-2">
          ...
          <div className="hidden sm:flex items-center gap-1.5 flex-wrap">
            {safeActiveFilters.categories?.map((category) => (
              ...
              >
                ...
                <button
                  onClick={() => {
                    const newCategories = safeActiveFilters.categories.filter((c) => c !== category)
            ...
            {Object.entries(safeActiveFilters.nutrition || {}).map(([key, value]) => {
            ...
            {getActiveFilterCount() > 5 && (
              <span className="text-xs text-muted-foreground">+{getActiveFilterCount() - 5} more</span>
            )}
          </div>
        </div>
        ...
        <div className="relative group z-10">
          <button className="flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-md border hover:bg-muted/50 transition-colors">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="16"
            ...
            {sortOptions.find((opt) => opt.id === safeActiveFilters.sort)?.label || "Default"}
          </button>
          <div className="absolute right-0 mt-1 w-48 p-1 rounded-md shadow-lg bg-card border hidden group-hover:block z-50">
      ...
      <AnimatePresence>
        {isExpanded && (
          ...
          >
            <div className="flex flex-col md:flex-row min-h-[300px] max-h-[70vh] overflow-hidden">
              {/* Sidebar Navigation */}
              <div className="md:w-48 border-b md:border-b-0 md:border-r bg-muted/30">
                <nav className="p-2">
                  <button
                  ...
                  >
                    ...
                    Nutrition Values
                    {safeActiveFilters.nutrition && Object.keys(safeActiveFilters.nutrition).length > 0 && (
                      <span className="ml-auto px-1.5 py-0.5 bg-primary/20 text-xs rounded-full">
              ...
              <div className="flex-1 p-4 overflow-y-auto">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {categories.map((category) => (
                        <button
                          key={category}
                          onClick={() => {
                            const currentCategories = safeActiveFilters.categories || []
                ...
                {activeSection === "nutrition" && (
                  <div className="space-y-4">
                    <h3 className="text-sm font-medium mb-2">Nutrition Values</h3>
                      ...
                      {nutritionRanges.map((range) => (
                        <div key={range.id} className="space-y-2">
                          ...
                          <input
                            ...
                            min={range.min}
                            max={range.max}
                            value={safeActiveFilters.nutrition?.[range.id] || range.min}
                            onChange={(e) => {
